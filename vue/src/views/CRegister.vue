            ...
            <el-form :model="corporation" :rules="rules" ref="corporationForm">
                ...
                <el-form-item style="margin: 5px 0; text-align: right">
                    ...
                    <el-button type="warning" size="small" autocomplete="off"
                        @click="$router.push('/Clogin')">返回登录</el-button>
                </el-form-item>
            </el-form>
        </div>
    </div>
    ...
    methods: {
        login() {
            this.$refs['corporationForm'].validate((valid) => {
                if (valid) {  // 表单校验合法
                    if (this.corporation.password !== this.corporation.confirmPassword) {
                        this.$message.error("两次输入的密码不一致")
                        return false
                    }
                    this.request.post("/corporation/cregister", this.corporation).then(res => {
                        if (res.code === '200') {
